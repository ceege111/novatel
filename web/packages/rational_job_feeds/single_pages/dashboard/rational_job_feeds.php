...
<?php  if( method_exists( $dashboardHelper, 'getDashboardPaneHeaderWrapper') ){  
	echo $dashboardHelper->getDashboardPaneHeaderWrapper(t('Job Feed Manager'),t("Create and configure rich-text feeds for your site, with clean URLs")); 
}else{ ?> 
    <h1><span><?php echo t('Job Feed Manager')?></span></h1> 
...
<?php  } ?> 
	...
	<?php  if( $mode=='configure' ){ ?>
		...
		<form id="save-rss-feed-form" action="<?php echo View::url($c->getCollectionPath().'/-/configure',intval($feedPage->getCollectionID())) ?>" method="post">
			...
			<h2><?php echo t('Configure Your Job Feed') ?></h2>
			...
			<table id="configureFeedData">
				<tr>
					...
					<td>
						<input name="company" type="text" value="<?php echo (strlen($_REQUEST['company'])) ? htmlentities($_REQUEST['company'], ENT_QUOTES, 'UTF-8') : htmlentities($feedPage->getCompany(), ENT_QUOTES, 'UTF-8'); ?>" />
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<select name="format">
						<?php foreach ($formats as $formatHandle => $formatName) { ?>
							<?php
								$curformat = $_REQUEST['format'] ? $_REQUEST['format'] : $feedPage->getFormat();
								$selected = $curformat == $formatHandle ? ' selected="selected"' : '';
							?>
							<option value="<?php echo $formatHandle ?>"<?php echo $selected; ?>>
								<?php echo htmlspecialchars($formatName); ?>
							</option>
						<?php }?>
						</select>
					</td>					
					...
					<td> 
						<select name="ctID">
							...
							<?php  foreach(CollectionType::getList() as $pageType ){ 
								if($pageType->getCollectionTypeID()==$jobFeedPageType->getCollectionTypeID()) continue; 
								?>
				...
				<tr>
					...
					<td> 
						<select name="orderBy">
							<?php  foreach(RationalJobFeed::$sortOptions as $key=>$val){ ?>
	...
	<?php  }else{ ?>
		...
		<div class="ccm-buttons">  
			<?php echo $ih->button(t('Create Job Feed'), $this->url($c->getCollectionPath().'/-/configure'), 'left')?>
		</div>	 		
		
		<div class="ccm-spacer">&nbsp;</div>
	
		<?php  if( !is_array($jobFeeds) || !count($jobFeeds) ){ ?>
		...
		<?php  }else{ ?>
			...
			<table id="feedsList">
				...
				<?php  foreach($jobFeeds as $jobFeed){ ?>
				<tr>
					<td><?php echo $jobFeed->getCollectionName() ?></td>
					<td><a href="<?php echo View::url($jobFeed->getCollectionPath()) ?>" target="_blank"><?php echo $jobFeed->getCollectionPath() ?></a></td>
					<td>
						<a href="<?php echo View::url($c->getCollectionPath().'/-/configure',$jobFeed->getCollectionID()) ?>"><?php echo t('Configure') ?></a>
							&nbsp;|&nbsp; 
						<a href="<?php echo View::url($c->getCollectionPath().'/-/delete',$jobFeed->getCollectionID()) ?>"><?php echo t('Delete') ?></a>
					</td>
				</tr>
				<?php  } ?> 
			</table>
