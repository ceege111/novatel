...
class TonyRssFeed extends Page {
	...
	public function loadRssData(){
		$db = Loader::db();
		$sql = 'SELECT * FROM TonyRssFeeds WHERE cID='. intval($this->getCollectionID());
		$data = $db->getRow($sql); 
	...
	public function save(){
		...
		if( $this->getRssID() ){ 
			$vals = array( $this->getCollectionID(), $this->getNumToDisplay(), $this->getFeedParentCID(), $this->getOrderBy(), $this->getResultsPageTypeID(), $this->getResultsUID(), $this->getDescSource(), $this->getDescSourceAreaHandle(), $this->getTruncateChars(), $this->getRichText(), $this->getCacheHours(), $this->includeChildren(), $this->getRssID() ); 
			$sql = 'UPDATE TonyRssFeeds SET cID=?, numToDisplay=?, cParentID=?, orderBy=?, ctID=?, uID=?, descSource=?, descSourceAreaHandle=?, truncateChars=?, richText=?, cacheHours=?, includeChildren=? WHERE rssID=?';
			return $db->query( $sql , $vals ); 
		}else{
			$vals = array( $this->getCollectionID(), $this->getNumToDisplay(),  $this->getFeedParentCID(), $this->getOrderBy(), $this->getResultsPageTypeID(), $this->getResultsUID(), $this->getDescSource(), $this->getDescSourceAreaHandle(), $this->getTruncateChars(), $this->getRichText(), $this->getCacheHours(), $this->includeChildren() );
			$result = $db->query( 'INSERT INTO TonyRssFeeds ( cID, numToDisplay, cParentID, orderBy, ctID, uID, descSource, descSourceAreaHandle, truncateChars, richText, cacheHours, includeChildren) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)' , $vals ); 
